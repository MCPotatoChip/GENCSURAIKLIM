...
const categories = ["Tümü", "Genel", "Etki", "Çözüm", "Proje", "Bilim"];
...
export default function SoruAtma() {
  ...
  const filtered = faqs.filter((faq) => {
    const matchCategory = activeCategory === "Tümü" || faq.category === activeCategory;
    const matchSearch =
      searchTerm === "" ||
      ...
      <main className="pt-32 pb-20 px-6 max-w-4xl mx-auto">
        ...
        <header className="mb-12 animate-fade-in-up">
          <h1 className="text-5xl md:text-6xl font-black tracking-tight mb-4 text-primary font-headline">
            Sıkça Sorulan <span className="text-on-surface-variant font-medium">Sorular</span>
          </h1>
          <p className="text-on-surface-variant text-lg leading-relaxed">
            İklim değişikliği, Genç İklim Şurası ve çevresel sorumluluklarımız hakkında en çok sorulan ve 
            yanıtları bilmeniz gereken kritik {faqs.length} soru.
          </p>
        </header>
              ...
              <>
                ...
                {categories.map((cat, idx) => (
                  <button
                    ...
                    className={`relative z-10 px-6 py-3 rounded-full text-sm font-bold uppercase tracking-widest cursor-pointer border-none outline-none transition-colors ${
                      activeCategory === cat
                        ? "text-on-primary"
                        : "bg-transparent text-on-surface-variant hover:text-on-surface"
                    }`}
                  >
                    {cat === "Tümü" ? `📚 Tümü (${faqs.length})` : cat}
                  </button>
                ))}
              </>
            );
        ...
        {filtered.length === 0 && (
          <div className="text-center py-20 bg-surface-container rounded-3xl border border-outline-variant/10 mt-8">
            ...
            <p className="text-on-surface-variant text-lg">Aramanızla eşleşen soru bulunamadı.</p>
          </div>
        )}
      </main>
    </div>
