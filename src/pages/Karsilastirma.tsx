...
export default function Karsilastirma() {
    ...
    const [city2Idx, setCity2Idx] = useState(8); // Default: İstanbul vs Londra
            ...
            <main className="min-h-screen pt-32 pb-20 px-6 max-w-7xl mx-auto space-y-12 animate-fade-in">
                ...
                <header className="text-center space-y-4 animate-fade-in-up">
                    <h1 className="text-5xl md:text-7xl font-black tracking-tighter text-on-surface font-headline">
                        Şehir İklim <span className="text-primary-container">Kıyaslaması</span>
                    </h1>
                    <p className="text-on-surface-variant max-w-2xl mx-auto text-lg font-body leading-relaxed">
                        Çoklu metriklerle iki farklı dünya şehrini yan yana karşılaştırın. Tüm veriler gelecekteki çevresel etki rotasını görselleştirir.
                    </p>
                </header>

                {/* City Search Shell */}
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
                    {citySelect(city1Idx, setCity1Idx, "Referans Şehir", "primary", "SEÇİM A")}
                    {citySelect(city2Idx, setCity2Idx, "Karşılaştırılan Şehir", "secondary", "SEÇİM B")}
                </div>
                ...
                <div className="flex justify-center w-full relative animate-scale-in" style={{ animationDelay: '0.2s' }}>
                    <div className="relative inline-flex flex-wrap md:flex-nowrap gap-2 bg-surface-container p-2 rounded-full border border-outline-variant/20 shadow-lg">
                        <div 
                            ...
                            style={{ 
                                left: `${metric === 'temp' ? 8 : metric === 'rain' ? 8 + 140 + 8 : 8 + 140 + 8 + 155 + 8}px`, // Fallback inline styling or just use React ref
                                // Wait! Let's just use refs using a nested component or just write the ref logic inside the Karsilastirma component.
                            }}
                        />
                        {/* We will implement the robust ref approach by mapping them */}
                        {(() => {
                            const [tabStyle, setTabStyle] = useState({ left: 8, top: 8, width: 0, height: 0, opacity: 0 });
                            const tabsRef = useRef<(HTMLButtonElement | null)[]>([]);
                            
                            const metrics = [
                                { key: "temp", label: "Sıcaklık", icon: "thermostat" },
                                { key: "rain", label: "Aylık Yağış", icon: "water_drop" },
                                { key: "humidity", label: "Nem Oranı", icon: "humidity_percentage" },
                            ];
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8 animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
                    ...
                    <div className="md:col-span-4 bg-surface-container p-8 rounded-2xl flex flex-col justify-between border border-outline-variant/10 shadow-lg">
                        <div className="flex justify-between items-start mb-8">
                            <h3 className="text-xl font-bold font-headline text-on-surface">Yıllık Ortalama</h3>
                    ...
                    <div className="md:col-span-8 glass-card p-6 md:p-8 rounded-2xl border border-outline-variant/30 relative overflow-hidden flex flex-col min-h-[400px]">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-xl md:text-2xl font-bold font-headline text-on-surface">Aylık Trend Karşılaştırması</h3>
                    ...
                    <div className="md:col-span-12 glass-card p-10 lg:p-12 rounded-3xl flex flex-col md:flex-row gap-12 items-center border border-primary/20 shadow-[0_0_40px_rgba(107,255,143,0.05)] biolume-glow">
                        <div className="md:w-1/3 text-center md:text-left space-y-6">
                            <h2 className="text-3xl md:text-4xl font-black leading-tight font-headline text-on-surface">Varyans<br />Analizi</h2>
                            <p className="text-on-surface-variant text-sm md:text-base leading-relaxed">
                                {city1.name} ve {city2.name} arasındaki sıcaklık, nem ve yağış metriklerindeki farklılıklar, coğrafi konumun ve okyanus akıntılarının mikro iklimleri nasıl şekillendirdiğini net bir şekilde gösteriyor.
                            </p>
                            <button className="bg-gradient-to-b from-primary to-primary-container text-on-primary-container font-black uppercase tracking-widest px-8 py-4 rounded-xl text-sm biolume-glow transition-all duration-300 hover:scale-105 active:scale-95 flex items-center justify-center gap-2 cursor-pointer w-full md:w-auto">
                                <span className="material-symbols-outlined">download</span> Raporu İndir
                            </button>
                        </div>
