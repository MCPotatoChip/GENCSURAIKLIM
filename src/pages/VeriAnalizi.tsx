...
export default function VeriAnalizi() {
  ...
  const sectorData = lang === 'tr' ? [
    { name: "Enerji", value: 72, color: "#6bff8f" },
    { name: "Tarım", value: 12, color: "#8ff8b4" },
    { name: "Sanayi", value: 9, color: "#fb923c" },
    { name: "Atık", value: 4, color: "#0ea5e9" },
    { name: "Diğer", value: 3, color: "#a1aea2" },
  ] : [
    { name: "Energy", value: 72, color: "#6bff8f" },
    { name: "Agriculture", value: 12, color: "#8ff8b4" },
    { name: "Industry", value: 9, color: "#fb923c" },
    { name: "Waste", value: 4, color: "#0ea5e9" },
    { name: "Other", value: 3, color: "#a1aea2" },
  ];
  ...
  const dataCards = lang === 'tr' ? [
    ...
    { icon: "thermostat", label: "Average Temp Increase", value: "+1.5°C", desc: "Last 50 years", color: "text-error" },
    { icon: "co2", label: "Annual CO₂ Emission", value: "490 Mt", desc: "2024 estimate", color: "text-primary" },
    { icon: "water", label: "Sea Level Rise", value: "+20 cm", desc: "Last 100 years", color: "text-secondary" },
    { icon: "forest", label: "Forest Loss", value: "10M ha", desc: "Annual global loss", color: "text-orange-400" },
    { icon: "solar_power", label: "Solar Energy Capacity", value: "12 GW", desc: "Turkey installed power", color: "text-primary-dim" },
    { icon: "factory", label: "Global CO₂", value: "420 ppm", desc: "Atmosphere density", color: "text-on-surface-variant" },
  ];

  const tabs = useMemo(() => [
    { key: "temp" as ChartTab, label: lang === 'tr' ? "Sıcaklık" : "Temperature", icon: "thermostat" },
    { key: "co2" as ChartTab, label: "CO₂", icon: "co2" },
    { key: "sector" as ChartTab, label: lang === 'tr' ? "Sektörel" : "Sectoral", icon: "pie_chart" },
  ], [lang]);
        ...
        <div className="bg-surface-container border border-outline-variant/30 p-4 rounded-xl shadow-[0_0_20px_rgba(0,0,0,0.5)]">
          ...
          {payload.map((entry: any, index: number) => (
            <p key={index} className="text-on-surface font-black text-lg flex items-center gap-2">
              ...
              {entry.value} {(entry.name.includes("Sıcaklık") || entry.name.includes("Temp")) ? "°C" : (entry.name.includes("Emisyonu") || entry.name.includes("Emission")) ? "Mt" : "%"}
            </p>
          ))}
        </div>
      );
  ...
  const text = lang === 'tr' ? {
    title1: "Veri",
    title2: "Analizi",
    desc: "Türkiye ve dünya çapındaki iklim verilerini görselleştirin. Emisyon trendlerini, sıcaklık değişimlerini ve sektörel dağılımı inceleyerek ekolojik kırılganlığımızı değerlendirin.",
    chartsTitle: "Detaylı Grafikler",
    tempChartTitle: "Türkiye Ortalama Sıcaklık Değişimi (1970 - 2024)",
    co2ChartTitle: "Türkiye Yıllık CO₂ Emisyonları (Mt)",
    sectorChartTitle: "Türkiye Sera Gazı Emisyonları - Sektörel Dağılım",
    tempName: "Ortalama Sıcaklık",
    co2Name: "CO₂ Emisyonu",
    sourceText: "Kaynak: Meteoroloji Genel Müdürlüğü, TÜİK ve Global Carbon Atlas verileri baz alınmıştır."
  } : {
    title1: "Data",
    title2: "Analysis",
    desc: "Visualize climate data across Turkey and the world. Evaluate our ecological vulnerability by examining emission trends, temperature changes, and sectoral distribution.",
    chartsTitle: "Detailed Charts",
    tempChartTitle: "Turkey Average Temperature Change (1970 - 2024)",
    co2ChartTitle: "Turkey Annual CO₂ Emissions (Mt)",
    sectorChartTitle: "Turkey Greenhouse Gas Emissions - Sectoral Distribution",
    tempName: "Average Temp",
    co2Name: "CO₂ Emission",
    sourceText: "Source: Based on data from the General Directorate of Meteorology, TURKSTAT, and Global Carbon Atlas."
  };
      ...
      <main className="pt-32 pb-20 px-6 max-w-7xl mx-auto">
        ...
        <header className="mb-12 animate-fade-in-up">
          <h1 className="text-5xl md:text-6xl font-black tracking-tight mb-4 text-on-surface font-headline">
            {text.title1} <span className="text-primary font-medium">{text.title2}</span>
          </h1>
